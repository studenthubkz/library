...
// Сброс database.json до начального состояния
try {
    // Создаем структуру с минимальным начальным состоянием
    const initialData = {
        "users": [
            {
                "email": "aiibidildaeva@gmail.com",
                "password": "admin",
                "name": "Админ"
            }
        ],
    ...
    // Записываем начальное состояние в файл database.json
    fs.writeFileSync(
        path.join(__dirname, 'database.json'), 
        JSON.stringify(initialData, null, 2),
        'utf8'
    );
    console.log('База данных database.json успешно очищена');
} catch (err) {
    console.error('Ошибка при очистке database.json:', err);
}
...
if (fs.existsSync(dbPath)) {
    ...
    const db = new sqlite3.Database(dbPath, (err) => {
        ...
        console.log('Соединение с базой данных library.db установлено');
        ...
        db.serialize(() => {
            ...
            // Выполняем серию SQL-запросов для удаления всех данных
            tables.forEach(table => {
                db.run(`DELETE FROM ${table}`, function(err) {
                    if (err) {
                        console.error(`Ошибка при очистке таблицы ${table}:`, err.message);
                    } else {
                        console.log(`Таблица ${table} успешно очищена. Удалено записей: ${this.changes}`);
                    }
                });
            });
            ...
            tables.forEach(table => {
                db.run(`DELETE FROM sqlite_sequence WHERE name='${table}'`, function(err) {
                    if (err) {
                        console.error(`Ошибка при сбросе автоинкремента для таблицы ${table}:`, err.message);
                    } else {
                        console.log(`Автоинкремент для таблицы ${table} сброшен`);
                    }
                });
            });
            ...
            db.close((err) => {
                if (err) {
                    return console.error('Ошибка при закрытии базы данных:', err.message);
                }
    ...
    console.log('Файл базы данных library.db не найден. Очистка не требуется.');
}